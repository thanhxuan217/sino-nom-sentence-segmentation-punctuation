...
def infer_batches(
    ...
    """Yield (raw_text, pred_text, pred_labels) for every sample in the
    streaming dataloader, running inference in batches for throughput.
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(batch_size):
                ...
                yield raw_text, pred_text, pred_labels
                total_samples += 1
...
def stitch_documents(
    fragment_stream: Generator[Tuple[str, str, List[str]], None, None],
    task_name: str,
    overlap: int = OVERLAP,
    logger: Optional[logging.Logger] = None,
) -> Generator[str, None, None]:
    """Consume a stream of (raw_text, pred_text, pred_labels) fragments
    and yield fully-stitched documents.
    ...
    Yields
    ------
    Completed document strings (predicted text with labels inserted).
    """
    ...
    for raw_text, pred_text, _pred_labels in fragment_stream:
        ...
        if prev_tail_raw == curr_head_raw:
            ...
            if valid_prev_pred and prev_pred:
                # Count how many raw chars the valid_prev_pred covers
                # starting from prev_overlap_start_raw.
                r2p_full = build_raw_to_pred_map(prev_raw, prev_pred)
                valid_end_pred = len(valid_prev_pred) + (
                    r2p_full[prev_overlap_start_raw] if prev_overlap_start_raw > 0 else 0
                )
                # Find the raw char index that corresponds to valid_end_pred.
                valid_raw_end = 0
                for ri, pi in enumerate(r2p_full):
                    if pi >= valid_end_pred:
                        valid_raw_end = ri
                        break
                else:
                ...
                kept_in_overlap = max(0, valid_raw_end - (len(prev_raw) - overlap))
...
def write_documents_jsonl(
    doc_stream: Generator[str, None, None],
    output_path: str,
    logger: Optional[logging.Logger] = None,
) -> int:
    """Stream completed documents to a JSONL file.

    Each line is a JSON object:  {"doc_id": <int>, "text": <str>}
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for doc_text in doc_stream:
            record = {"doc_id": total, "text": doc_text}
...
def main():
    ...
    # Step 3: Stream documents to JSONL output.
    output_path = os.path.join(
        args.output_dir,
        f"{args.split}_{args.task}_stitched.jsonl",
    )
    total_docs = write_documents_jsonl(doc_gen, output_path, logger=logger)
